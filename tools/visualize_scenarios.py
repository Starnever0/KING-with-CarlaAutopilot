...
class BEVVisualizer:
    ...
    def visualize(self):
        ...
        for log_path, results_path in tqdm(
            ...
            # Robustly find density from path
            density = "4"
            for part in log_path.split("/"):
                if part.startswith("agents_"):
                    density = part.split("_")[-1]
                    break

            # 路径由1改成2，因为传入的是./generation_results/agents_4/...而不是generation_results/agents_4/...
            critical_iter = results["first_metrics"]["iteration"]

            if self.args.opt_iter == -1:
                # Check if density and town exist in max_iter to avoid KeyError
                if density in self.max_iter and town in self.max_iter[density]:
            ...
            # 修改保存路径，使用前缀 "vis_" 避免与 RouteScenario 目录模式冲突
            save_path = os.path.join(os.path.dirname(log_path), f"vis_{name}_iter_{opt_iter}")
